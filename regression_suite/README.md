...
- **Exact event count.** The candidate must emit exactly as many rows as the reference, and the emitted row count is the ranked numerator, so a padded or truncated trace is refused here. Note what "deterministic" does and does not mean: a scenario is deterministic *for a given implementation and seed*, not across implementations. 65 of the 66 public scenarios declare a stochastic latency model, and those draws come from one seeded NumPy global stream whose position depends on the number of agents constructed before it. A candidate that changes the RNG, the number of draws, or their order gets a different latency stream and a different event calendar from the same seed. See the Tier-A note in the top-level `README.md`.
